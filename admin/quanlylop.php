                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-flex align-items-center justify-content-between">
                                <h4 class="mb-0 font-size-18">QUẢN LÝ LỚP HỌC</h4>
...
if (isset($_GET['s']) && $_GET['s'] != '') {
	$sql = 'select * from class where fullname like "%'.$_GET['s'].'%"';
} else {
...
foreach ($classList as $std) {
	echo '<tr>
			...
			<td><button class="btn btn-danger" onclick="deleteClass('.$std['ID'].')">Delete</button></td>
		</tr>';
}
?>
					</tbody>
				</table>
				<button class="btn btn-success" onclick="window.open('lop.php', '_self')">Add Class</button>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		function deleteClass(ID) {
			option = confirm('Bạn có muốn xóa lớp học này không')
			if(!option) {
				return;
			}
