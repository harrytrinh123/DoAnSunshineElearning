                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-flex align-items-center justify-content-between">
                                <h4 class="mb-0 font-size-18">QUẢN LÝ KHÓA HỌC</h4>
...
if (isset($_GET['s']) && $_GET['s'] != '') {
	$sql = 'select * from course where fullname like "%'.$_GET['s'].'%"';
} else {
...
$index = 1;
foreach ($courseList as $std) {
	echo '<tr>
            <td>'.($index++).'</td>
			<td>'.$std['CourseName'].'</td>
			<td>'.$std['Description'].'</td>
            
			<td><button class="btn btn-warning" onclick=\'window.open("kh.php?ID='.$std['ID'].'","_self")\'>Edit</button></td>
			<td><button class="btn btn-danger" onclick="deleteCourse('.$std['ID'].')">Delete</button></td>
		</tr>';
}
?>
					</tbody>
				</table>
				<button class="btn btn-success" onclick="window.open('kh.php', '_self')">Add Course</button>
			</div>
		</div>
	</div>
